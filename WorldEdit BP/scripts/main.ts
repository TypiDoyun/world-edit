import { DimensionLocation, Player, Vector3, system, world } from "@minecraft/server";
...
system.runInterval(() => {
    ...
    for (const player of players) {
        ...
        if (!posState.isValid) continue;

        const drawDot = (vector: Vector3) => {
            const direction = {
            ...
            if (distanceSquared > 1600) return;
            ...
            if (rightResult > 0 || leftResult < 0) return;

            player.spawnParticle(particleId, vector);
        };

        const drawFace = (type: "x" | "y" | "z", vector: Vector3) => {
            const direction = {
                x: vector.x - player.location.x,
                y: vector.y - player.location.y,
                z: vector.z - player.location.z
            ...
            if (distanceSquared > 2500) return;
