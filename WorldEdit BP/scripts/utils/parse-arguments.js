...
const getPrimitiveType = (type) => {
    ...
    if (isType("String"))
        return "String";
    else if (["Number", "PositionX", "PositionY", "PositionZ", "BlockPositionX", "BlockPositionY", "BlockPositionZ"].some(t => isType(t)))
        return "Number";
    else if (isType("Boolean"))
        return "Boolean";
...
export const parseArguments = (origin, command, rawArguments) => {
    const args = [];
    const types = [];
    let tempArgument = "";
    let isQuoted = false;
    ...
    const pushTemp = () => {
        ...
        if (tempArgument.startsWith("@")) {
            ...
            argumentType = "String";
            if (positionReg.test(tempArgument))
                argumentType = "Number";
            convertedArg = tempArgument;
        }
        ...
        if (argumentType === "Player") {
            ...
            if (positionReg.test(convertedArg) && (requiredType.startsWith("Position") || requiredType.startsWith("BlockPosition"))) {
                const axis = requiredType.slice(-1).toLowerCase();
                const value = convertedArg.match(positionReg)[1];
                convertedArg = origin.location[axis] + (value === undefined ? 0 : +value);
                if (requiredType.startsWith("Block"))
                    convertedArg = Math.floor(convertedArg);
            }
        }
    ...
    if (minLength > args.length) {
        if (!isErrorOccurred)
            sendMessage(origin, `Syntax error: Unexpected "": at argument ${args.length + 1}"`, "c");
        isErrorOccurred = true;
    }
    return {
